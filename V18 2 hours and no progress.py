    ...
    def iterative_cycle(self):
        ...
        with open('HackathonOutput.csv', 'w', newline='') as file:
            ...
            for pair in allcols:
                ...
                for n in self.tag:
                    results.append(n) ## new trial at dealing with problems
                ## Adds tag to end -> Mess with joining at end during cleanup
                writer.writerow(results) ## Writes in
                

    def cleaning_cycle_1(self): #done with pandas - cant create a dataframe to save my life - 2 hours dead
         with open("HackathonOutput.csv", 'r') as f:
             datas = list(csv.reader(f))
             data = np.array(datas[1:])
             print(data)
             print(data[2,-1])
