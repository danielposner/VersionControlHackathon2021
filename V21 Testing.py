...
## Use this later - not for demonstration
#openme = input("Type the file name - no error implementation no punctuation")
#with open(openme, 'r') as f:

## USE THE BELOW ONE FOR SUBMISSION
...
####################################################################################################
    ...
    def _liner_regression_model(self, a,b):
        ...
        #self.tag.append("0")
        #self.tag.append("0")
        if x[2] > .3 or x[2] < -3: #corr coeff medium
            self.tag.append("1")
        else:
            self.tag.append("0")
        if x[3] < 0.05:
...
## Removed useless data
        ...
        for n in self.full_list: #### RECHECK TAGS FOR ALL SETS
            ...
            if n == self._pearson_correlation_test:
                special.append("Pearson Coefficient") ## No data screening - outliers have large effect
