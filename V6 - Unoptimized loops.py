...
class Main:
    ...
    def iterative_cycle(self): ## First set up functionality here -> then focus on optimizing using NUMPY kit
        for column in rangecols[:11]:
            #print(wines[:,column]);
            #print(wines[:,column+1]);
            _pearson_correlation_test(wines[:,column], wines[:,column+1])
