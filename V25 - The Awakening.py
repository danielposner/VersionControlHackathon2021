...
####################################################################################################
    ...
    def correlation_plot1(self):
        df = pd.DataFrame(wines, columns=[names])
    ...
    def correlation_plot2(self):
        df = pd.read_csv("HackathonOutput.csv")
        plt.figure(figsize=(10,8), dpi= 80)
        sns.heatmap(df.corr(), xticklabels=df.corr().columns, yticklabels=df.corr().columns, cmap='RdYlGn', center=0, annot=True)
        plt.title('''Correlogram of Statistical Test
                  s ## BAD TOOL - Interesting but do not rely
                  on this''', fontsize=22)
