...
def _single_covariance_test():
    """Tests two columns for covariance"""
    ## May way to test all rows and columns for a covariance matrix later on!!
    print(wines[:,3])
    cov_mat = np.stack((wines[:,3], wines[:,5]), axis = 0)
    print(np.cov(cov_mat))


_single_covariance_test()
